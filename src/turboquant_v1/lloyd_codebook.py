...
## conditional expectation (mean) of the random variable
...
def compute_lloyd_max_codebook(d: int, 
                               bits: int, 
                               max_iter: int = 200, 
    ...
    pdf_values = beta_pdf(x_grid,d)

    cdf_values = np.cumsum(pdf_values) * (x_grid[1] - x_grid[0])
    #To make the distribution strictly E [-1,1]
    ...
    for i in range(n_clusters):
        ...
        # first index where CDF ≥ q_mid
        idx = np.searchsorted(cdf_values, q_mid)
