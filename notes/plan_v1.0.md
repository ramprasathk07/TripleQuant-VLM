...
**The real hero result (now trustworthy):** on the corrected data, fp16 baseline's context sweep tops out at 4096 tokens (12GB card), torchao-int8wo *also* caps at 4096 in this sweep's grid (real ~1GB headroom at every matched length, e.g. 5324MB vs fp16's 6364MB at 4096 tokens — just not enough to survive the next power-of-2 step; both fail at 8192), and **TurboQuant K3V2 reaches 16384 — 4x fp16/torchao's usable context on the same GPU.** This is the actual, defensible TurboQuant value proposition and the chart to lead the README with (`results/qwen3-1.7b-sweep/report/plots/ctx_sweep.png`). Correction 2026-07-17 (later same session): the original version of this note claimed torchao reached 8192 — misread from the chart rather than the underlying `max_fit_tokens` field. Caught when the user asked whether the README's numbers were real; verifying against raw JSON found the error. Fixed here and in README.md / docs/benchmark_report.md.
...
## 10. M3 completion notes (2026-07-17)

Done same session as M0-M2. Scope followed §5 closely, plus a few things found along the way:

**README rewrite:** hero results (leaderboard + the 4x-context chart) now leads; TurboQuant's math moved from mid-page into the top third; Decision Matrix and Compatibility Matrix added, both built strictly from what's implemented/measured — not the aspirational version. Caught and correctly documented as *not wired* rather than overclaimed: TurboQuant x vLLM (`src/turboquant_v1/vllm_backend.py` imports `turboquant.*`, which doesn't resolve in this repo — verified with a direct import before writing the claim), torchao x vLLM (schema comment itself says "HF only").

**docs/ built as curated front door, not a duplicate of notes/:** `benchmark_report.md` (M2's report + narrative annotations explaining what the flattened table can't show), `failure_cases.md` (8 entries — the WDDM bug, the K3V2 tradeoff, QJL, teacher-forced-PPL blindness, FP8/Ampere, ModelOpt/Windows, vLLM split-env, unbuilt Triton kernels), `experiments.md` (chronological sweep history, including which bugs were hiding in which historical run), `engineering_notes.md` (index into `notes/debugging_turboquant_kv.md`'s "reusable toolkit," not a copy — plus a note that the WDDM bug and the teacher-forced-PPL blindness are the same underlying mistake — "storage vs. compute path" — recurring at a different layer a month apart).

**Repo hygiene, with one real bug found:** `req.txt` removed (stale freeze, torch 2.2.2+cu118 vs the actual 2.8.0+cu128 stack — not referenced anywhere). `serve.py` removed (empty). Setup scripts collapsed (all `*.bat` are gitignored, so this was local-only cleanup, invisible to git — `setup.sh` is the one tracked script and needed no changes). **Found:** both `setup.sh` and the newer `setup1.bat` already called `pip install -e ".[vllm]"`, but `pyproject.toml` never defined a `vllm` extra — `vllm` was a hard base dependency instead, contradicting the README's own "run vLLM separately" guidance and making that pip command a guaranteed hard-error. Fixed: `vllm`/`modelopt`/`torchao` are now proper optional extras. Also added the missing `LICENSE` file (Apache-2.0 — already the declared license in `pyproject.toml` and both README versions, just never existed as a file).

**Not done / deferred:** no `.[vllm]` extras install was actually run to verify end-to-end (no second Windows env for it in this session) — flagged honestly in the Compatibility Matrix rather than assumed working.
